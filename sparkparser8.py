...
def find_UUID(text="UUID"):
    x = get_next_data() # should be 0x59
    x = get_next_data() # should be length 0x24
    s_str = ""
        ...
        if m_cmd == 0x01:
            if m_param == 0x01:  # send whole preset
                ...
                if chunk_this == 0:       # first chunk of preset
                    ...
                    pedals = find_byte("Number of pedals in preset") - 0x10
                    for r in range(0, pedals):
                        find_string("Pedal  ")
                        find_boolean()
                        vals = find_byte("Parameters for this pedal") - 0x10
